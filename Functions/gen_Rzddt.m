...
%         for n1 = 0:N-1
%             for n2 = 0:N-1
%                 R_ztf(m1*N+n1+1,m2*N+n2+1) = ambig_direct((n1-n2)*Ts,(m2-m1)/Ts,Ts,shape,alpha,q,res_discretization);
%             end
%         end
%     end
%     fprintf("x")
...
% R_zt = kron(eye(N),F_M)' * R_ztf * kron(eye(N),F_M);
...
% % Generate noise whitening matrix
% [V_z,Omega_z,~] = svd(R_zdd_tilde);
% D = sqrt(inv(Omega_z)) * V_z';
